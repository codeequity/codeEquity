...
class Allocation {
   ...
   Allocation({required this.category, this.categoryBase, this.amount, this.sourcePeq,
            required this.allocType, this.ceUID, required this.hostUserName, this.vestedPerc, this.notes, this.ghProjectId });
   ...
   dynamic toJson() => {'Category': category, 'CategoryBase': categoryBase, 'Amount': amount, 'SourcePEQ': sourcePeq, 'AllocType': enumToStr(allocType),
                           'CEUID': ceUID, 'HostUserName': hostUserName, 
   ...
   String toString() {
      ...
      res += "\n    " + enumToStr( allocType ) + " ceUID and hostUserName: " + (ceUID ?? "") + " " + hostUserName;
