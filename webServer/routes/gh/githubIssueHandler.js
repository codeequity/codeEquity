...
// Terminology:
// situated issue: an issue located in a CE-valid project structure
// carded issue:   an issue located in one or more projects (valid CE or not)
// newborn issue:  a plain issue not located in a project without PEQ label
// draft issue:    not yet an issue.  Similar to newborn card in classic.

// Guarantee: Once populateCEProjects has been run once for a repo:
//            1) Every carded issue in that repo resides in the linkage table.
//            2) Newborn issues and draft issues can still exist (pre-existing, or post-populate), and will not reside in the linkage table.
//            3) label operation on newborn issues will cause conversion to carded (unclaimed) and inclusion in linkage table.
...
//
// XXX pd will either be gh2Data, or ghCData.  Should be ghData, then add on as needed for gh2, ghc.
// XXX pd.GH vs pd.Host
// XXX rewrite all so that 1:1 issue:card -> issue:loc where loc is loc in project column.. status in gh2.  no status is 'no status' loc/column.
//                                           i.e. 1 issue in 1 repo in 1 project in 1 column.  GH will allow 1 issue in n projects.
